...
const Users = () => {
  ...
  const { data: users, isLoading } = useQuery({
    ...
    queryFn: () =>
      selectedOrganization ? userService.getAll(selectedOrganization.organizationId) : Promise.resolve([]),
  ...
  return (
    <Container maxW={'6xl'} py={4}>
              ...
              {users && users.length > 0 ? (
                users.map((user) => (
                  <tr key={user.userId} className="hover:bg-stone-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-stone-500">{user.password}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-stone-500">{user.roleId}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-stone-500">{user.areaId}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-stone-500">{user.depotId}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
      ...
      >
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="mx-auto max-w-md rounded bg-white p-6">
            <Dialog.Title className="text-lg font-medium mb-4">Create User</Dialog.Title>
            <UserForm
      ...
      >
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="mx-auto max-w-md rounded bg-white p-6">
            <Dialog.Title className="text-lg font-medium mb-4">Edit User</Dialog.Title>
            {editingUser && (
              <UserForm
